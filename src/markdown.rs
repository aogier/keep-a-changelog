...
use pulldown_cmark::{Alignment, CodeBlockKind, Event, InlineStr, LinkType, Tag};
    ...
    fn start_tag(&mut self, tag: Tag<'a>) -> io::Result<()> {
        match tag {
            ...
            Tag::Link(_link_type, dest, title) => {
                ...
                if let Some(Event::Text(tag)) = next {
                    self.write(&format!("[{}]({}", tag, dest))?
                }
                // self.write("ST LINK")?;
                return Ok(());
            }
